...
namespace Implementation.Logging
{
    public class ConsoleUseCaseLogger : IUseCaseLogger
    {
        ...
        public void Log(IUseCase useCase, IApplicationActor actor, object firstData, object secondData)
        {
            Console.WriteLine($"{DateTime.Now}: {actor.Identity} is trying to execute {useCase.Name} using data: " +
                $"{JsonConvert.SerializeObject(firstData)} id and {JsonConvert.SerializeObject(secondData)} id");
        }
    }
}
